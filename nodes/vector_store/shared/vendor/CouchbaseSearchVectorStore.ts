/*
 * Vendored from @langchain/community (MIT, Copyright (c) LangChain, Inc.)
 *   src/vectorstores/couchbase_search.ts
 *
 * Why this is vendored rather than imported:
 *
 * n8n exposes its own node_modules to community nodes via NODE_PATH, and checks like
 * `vectorStore instanceof VectorStore` in n8n's RetrieverVectorStore compare against
 * *n8n's* copy of @langchain/core. A store built from a second, bundled copy of core
 * fails that check, and n8n then mis-handles it — which broke the vector stores'
 * "retrieve" mode entirely.
 *
 * Importing @langchain/community from n8n instead does not work either: its
 * couchbase_*.cjs does `require('couchbase')` from n8n's module tree, and n8n does not
 * ship the Couchbase driver.
 *
 * Vendoring satisfies both constraints — this class extends n8n's @langchain/core
 * (an optional peer dependency) while using this package's own `couchbase` driver.
 *
 * Keep in sync with upstream when @langchain/community changes these files.
 */
